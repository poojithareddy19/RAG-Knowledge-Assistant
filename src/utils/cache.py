...
def _key(
    ...
    """Hash the inputs that change the SQL, ``version`` included.

    The prompt is one of those inputs: when it changes, entries generated by
    the old prompt must not be served, so the caller passes a version string
    that moves with it.
    """
